...
FSGJJ_GWHWJ_INDEX = "https://fsgjj.org.cn/gwhwj/index.jhtml"
...
def _page_url(page_no: int) -> str:
    if page_no <= 1:
        return FSGJJ_GWHWJ_INDEX
    return f"https://fsgjj.org.cn/gwhwj/index_{page_no}.jhtml"
...
def _extract_page_items(page_url: str, html: str, now: datetime, since_date) -> tuple[list[dict], bool]:
    ...
    for li in soup.select("div.itemListPage ul.list-date-div5 li"):
        a_tag = li.select_one("div.title a[href]")
        date_tag = li.select_one("div.date")
...
def crawl_fsgjj_gwhwj(current_time: datetime | None = None, max_pages: int = 14) -> list[dict]:
    """抓取抚顺市住房公积金管理中心政策法规/管委会文件栏目，仅保留近24小时内条目。"""
    ...
    try:
        first_resp = session.get(FSGJJ_GWHWJ_INDEX, timeout=20)
        first_resp.encoding = first_resp.apparent_encoding or "utf-8"
        if first_resp.status_code != 200:
            print(f"[FSGJJ] HTTP Error {first_resp.status_code}")
            return []
    except Exception as e:
        print(f"[FSGJJ] fetch failed(page=1): {e}")
        return []
        ...
        if page_no == 1:
            ...
            try:
                ...
                if resp.status_code != 200:
                    print(f"[FSGJJ] HTTP Error {resp.status_code} @ page={page_no}")
                    break
                html = resp.text
            except Exception as e:
                print(f"[FSGJJ] fetch failed(page={page_no}): {e}")
                break
