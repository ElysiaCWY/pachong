...
JMSGJJ_INDEX = "https://www.jmsgjj.org.cn/bzbwz/zcfg/index.jhtml"
...
def _page_url(page_no: int) -> str:
    if page_no <= 1:
        return JMSGJJ_INDEX
    return f"https://www.jmsgjj.org.cn/bzbwz/zcfg/index_{page_no}.jhtml"
...
def _extract_page_items(page_url: str, html: str, since_date: date) -> tuple[list[dict], bool]:
    ...
    for a_tag in soup.find_all("a", href=True):
        ...
        if not re.search(r"/bzbwz/zcfg/\d+\.jhtml$", full_url):
            continue
...
def crawl_jmsgjj_zcfg(current_time: datetime | None = None, max_pages: int = 4) -> list[dict]:
    """抓取佳木斯住房公积金政策法规，仅保留近24小时内发布的标题和链接。"""
    ...
    for page_no in range(1, max_pages + 1):
        ...
        try:
            ...
            print(f"[JMSGJJ] fetch failed(page={page_no}): {e}")
            break
