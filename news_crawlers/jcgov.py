...
INDEX_URL = "https://jcgov.gov.cn/bsfw/jcsgjjw/zcfg_48657/"
...
def _extract_items(page_url: str, html: str) -> list[dict]:
    ...
    for a in soup.find_all("a", href=True):
        try:
            ...
            # 仅保留路径包含本栏目标识的链接（防止抓到外部或导航链接）
            if '/bsfw/jcsgjjw/zcfg_48657/' not in full:
                # 有时候链接是 /bsfw/jcsgjjw/zcfg/xxx.html，也允许
                if '/bsfw/jcsgjjw/' not in full:
                    continue

            # 尝试从标题或父容器文本中解析日期 YYYY-MM-DD
            dt = None
...
def crawl_jcgov_policy(current_time: datetime | None = None, max_pages: int = 3) -> list[dict]:
    """抓取晋城政务网 - 政策文件栏目（返回 title/url/date/source 列表）。外层负责 24 小时过滤。"""
    ...
    for page_no in range(1, max_pages + 1):
        ...
        try:
            ...
            print(f"[JC GOV] Crawl error(p{page_no}): {e}")
            break
