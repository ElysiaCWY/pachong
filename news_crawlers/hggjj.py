...
HGGJJ_INDEX = "https://www.hggjj.org.cn/zxwj/index.jhtml"
...
def _page_url(page_no: int) -> str:
    if page_no <= 1:
        return HGGJJ_INDEX
    return f"https://www.hggjj.org.cn/zxwj/index_{page_no}.jhtml"
...
def _extract_page_items(page_url: str, html: str, since_date: date) -> tuple[list[dict], bool]:
    ...
    for a_tag in soup.find_all("a", href=True):
        ...
        if match and match.group(1) in title:
            title = norm(title.replace(match.group(1), "", 1))
        if not title:
            continue
...
def crawl_hggjj_zxwj(current_time: datetime | None = None, max_pages: int = 5) -> list[dict]:
    """抓取鹤岗市住房公积金管理中心中心文件，仅保留近24小时内发布的标题和链接。"""
    ...
    for page_no in range(1, max_pages + 1):
        ...
        try:
            ...
            print(f"[HGGJJ] fetch failed(page={page_no}): {e}")
            break
