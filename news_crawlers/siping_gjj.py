...
INDEX_URL = "http://gjj.siping.gov.cn/zcfg/gwhjzxwj/"
HOME_URL = "http://gjj.siping.gov.cn/"
PAGE_FMT = "http://gjj.siping.gov.cn/zcfg/gwhjzxwj/index_{page}.html"
...
def _extract_items(page_url: str, html: str) -> list[dict]:
    ...
    for a in soup.select("a[href]"):
        try:
            ...
            # only detail pages matching tYYYYMMDD_xxx.html
...
def crawl_siping_gjj_policy(current_time: datetime | None = None, max_pages: int = 5) -> list[dict]:
    """抓取四平市管委会及中心文件的标题与链接，返回带日期字段的条目列表（用于近24小时过滤）。"""
    ...
    for page_no in range(1, max_pages + 1):
        ...
        try:
            ...
            print(f"[Siping GJJ] Crawl error(p{page_no}): {e}")
            break
