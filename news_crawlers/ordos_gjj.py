...
ORDOS_GJJ_INDEX = "http://ordosgjj.org.cn/zcfg202405/zcfgnr202405/zxgdw/"


def _page_url(page_no: int) -> str:
    if page_no <= 1:
        return ORDOS_GJJ_INDEX
    return f"http://ordosgjj.org.cn/zcfg202405/zcfgnr202405/zxgdw/index_{page_no - 1}.html"
...
def _extract_page_items(page_url: str, html: str, since_date: date) -> tuple[list[dict], bool]:
    ...
    for a_tag in soup.find_all("a", href=True):
        ...
        if not href:
            continue
        if not (
            re.search(r"/zcfg202405/zcfgnr202405/zxgdw/\d{6}/t\d+(?:_\d+)?\.html$", href)
            or re.search(r"^\./\d{6}/t\d+(?:_\d+)?\.html$", href)
        ):
            continue
...
def crawl_ordos_gjj_zxgdw(current_time: datetime | None = None, max_pages: int = 3) -> list[dict]:
    """抓取鄂尔多斯住房公积金-中心规定文件，仅保留近24小时内条目。"""
    ...
    for page_no in range(1, max_pages + 1):
        ...
        try:
            ...
            print(f"[ORDOS_GJJ] fetch failed(page={page_no}): {e}")
            break
