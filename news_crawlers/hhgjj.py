...
HHGJJ_INDEX = "https://www.hhgjj.org.cn/newslistnew.jsp?navid=4&listid=27"
...
def _page_url(page_no: int) -> str:
    if page_no <= 1:
        return HHGJJ_INDEX
    return f"https://www.hhgjj.org.cn/newslistnew.jsp?navid=4&listid=27&page={page_no}"
...
def _extract_page_items(page_url: str, html: str, since_date: date) -> tuple[list[dict], bool]:
    ...
    for a_tag in soup.find_all("a", href=True):
        ...
        if "newsDetailsnew.jsp?newsid=" not in href:
            continue
...
def crawl_hhgjj_zxwj(current_time: datetime | None = None, max_pages: int = 5) -> list[dict]:
    """抓取黑河市住房公积金管理中心中心文件，仅保留近24小时内发布的标题和链接。"""
    ...
    for page_no in range(1, max_pages + 1):
        ...
        try:
            ...
            print(f"[HHGJJ] fetch failed(page={page_no}): {e}")
            break
